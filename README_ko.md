...
| 의존성 | 없음. `nlohmann/json` 3.12.0이 번들로 포함됩니다. | 코어: 없음(표준 라이브러리만). 소프트웨어 업데이트 유틸리티는 선택적으로 `cryptography`가 필요합니다 — `pip install afi-sdk[update]`. |
...
| `sample_shii_watchdog.py` | SHII 경고가 연속되면 센서 재시작 |
| `sample_stream_reconnect.py` | RDI 스트림이 끊기면 자동 재연결 |
| `sample_config_backup.py` | 센서 설정을 JSON으로 저장/복원 |
...
소프트웨어 번들로 센서를 플래시합니다. 유틸리티가 센서를 탐색해 병렬로(최대 8대)
업데이트하며, 각 센서마다 번들의 도메인별 git 버전과 센서가 보고하는 해시를 비교해
**다른 도메인만** 플래시한 뒤 재부팅·검증합니다.

```bash
pip install afi-sdk[update]        # 복호화에는 'cryptography' extra가 필요합니다
python tools/software_update.py <bundle.tar> --password <pw>
```

- 번들 = `software_manifest.json` + 도메인별 `bootloader/driver/firmware` 패키지
  (`*.tar.gz.enc`)를 담은 `.tar` 파일입니다.
- 번들 password는 SDK에 포함되지 않습니다 — `--password` 또는
  `AFI_UPDATE_PASSWORD` 환경변수로 전달하세요.
- 두 센서가 같은 IP로 응답하면, 도구는 중단하고 먼저 고유 IP를 설정하라고
  안내합니다(`set_network_config`).
- 세 도메인(bootloader/driver/firmware) 모두 해시를 비교해 **다를 때만** 플래시합니다.
  `--force`는 변경 없는 도메인도 강제로 재플래시합니다.
  `--no-reboot` / `--no-verify`로 재부팅·검증을 생략할 수 있습니다.

자세한 내용은 [API 레퍼런스](docs/api_reference.md#software-update)를 참고하세요.
...
프레임당 최대 **8192개 detection**(36바이트 헤더 + detection당 51바이트)을 포함합니다. 큰 RDI 프레임은 SOME/IP-TP로 분할 전송되며 SDK가 자동으로 재조립합니다.
...
- **소프트웨어 업데이트**는 동일한 평문 Config API로 전송되며 센서 펌웨어를 플래시합니다. 신뢰된 격리 네트워크에서만 실행하세요. 업데이트 번들은 암호화돼 있으며, 번들 password는 비밀로 유지하고 password나 번들 내용을 소스 저장소에 절대 커밋하지 마세요.

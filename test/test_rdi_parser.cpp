...
static std::vector<uint8_t> make_rdi(uint16_t n) {
    ...
    pack_le16(&buf[32], 4096);                    // capability
...
TEST(layout_constants) {
    ...
    CHECK(BTS_RDI_MAX_DETECTIONS == 4096u);
...
TEST(parse_empty_frame) {
    ...
    CHECK(frame.message.recognised_detections_capability == 4096);
...
TEST(count_overflow_clamped) {
    auto buf = make_rdi(1);
    pack_le16(&buf[34], 5000);  // num_detections > max 4096

    RdiParser parser;
    AfiRdiFrame frame{};
    CHECK(parser.Parse(buf.data(), buf.size(), frame) == 0);
    CHECK(frame.message.num_detections <= 4096);

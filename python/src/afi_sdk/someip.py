...
# Software Update (0x008x) — bootloader/driver/firmware flashing over Config API
METHOD_SW_UPDATE_START          = 0x0080
METHOD_SW_DATA_CHUNK            = 0x0081  # raw binary payload (not JSON)
METHOD_SW_UPDATE_COMMIT         = 0x0082
METHOD_SW_UPDATE_STATUS         = 0x0083
METHOD_SW_UPDATE_ABORT          = 0x0084
METHOD_SW_SWITCH_SLOT           = 0x0085
METHOD_SW_UPDATE_REBOOT         = 0x0086
METHOD_GET_BOOT_INFO            = 0x0087
...
class ConfigClient:
    ...
    def request(self, method_id: int, payload: Optional[dict] = None
        ...
        return self._transact(method_id, body)

    def request_binary(self, method_id: int, raw: bytes
                       ) -> Tuple[int, Optional[dict]]:
        """Send a SOME/IP request whose body is a raw binary payload.

        Used by the software-update chunk method (0x0081), which carries a raw
        binary firmware chunk (not JSON) but still returns a JSON response.

        Args:
            method_id: Config API method ID
            raw: raw binary body bytes

        Returns:
            Tuple of (return_code, response_data_dict_or_None), same as request().
        """
        return self._transact(method_id, raw)

    def _transact(self, method_id: int, body: bytes
                  ) -> Tuple[int, Optional[dict]]:
        """Send one SOME/IP request with an arbitrary body and parse the
        (JSON) response. Shared by request() and request_binary()."""
        if not self._sock:
            return (-1, None)

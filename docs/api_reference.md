...
- Large RDI frames (up to 8192 detections) are segmented using SOME/IP-TP and reassembled automatically by the SDK.
...
Up to **8192 detections per frame** (36-byte detection header + 51 bytes per detection). The ambiguity domain covers radial velocity only (`radial_velocity_begin` / `radial_velocity_end` in the header). Large RDI frames are segmented using SOME/IP-TP and reassembled automatically by the SDK.
...
Flash a sensor's `bootloader`, `driver`, and `firmware` from a software bundle
over the Config API (SOME/IP methods `0x0080`–`0x0087`). Exposed in Python via
`afi_sdk.software_update` and driven by the `tools/software_update.py` CLI.

Decrypting bundles requires the `cryptography` extra:

```bash
pip install afi-sdk[update]
```

### Bundle format

A software bundle is a plaintext `.tar` containing a two-level manifest:
...
Each per-domain `*.tar.gz.enc` decrypts (PBKDF2-HMAC-SHA256, 100k iterations →
AES-256-CBC) to a `*.tar.gz` holding a per-domain `manifest.json`
(`update_type`, `git_version`, `components[]` with per-component `crc32`) and the
component binaries. A single-domain package (`firmware_*.tar.gz.enc`) may also be
flashed directly.
...
| Option | Description |
|--------|-------------|
| `--password` / `AFI_UPDATE_PASSWORD` | Bundle decryption password (required for encrypted bundles; never shipped with the SDK) |
| `--ip A,B` | Target specific IPs (default: all discovered) |
| `--bind-ip` | Local IP to bind for discovery broadcast (multi-NIC hosts) |
| `--force` | Flash every domain even if its hash is unchanged |
| `--components a,b` | Flash only these components (partial; disables the skip) |
| `--no-reboot` / `--no-verify` | Skip the post-flash reboot / verification |
| `--timeout` | Per-component flash timeout in seconds (default 120) |
| `--no-progress` | Plain line output instead of the live in-place display |

Behavior:

- **Discovery-driven, parallel** — discovers sensors and updates up to 8 at once.
- **Per-domain progress** — the live display shows each sensor's current domain as
  an `[i/N]` counter over the domains being applied (e.g. `[1/3] bootloader`,
  `[2/3] driver`, `[3/3] firmware`) plus the transfer/flash percentage.
- **Duplicate-IP guard** — if two sensors answer on the same IP, the tool prints
  the conflict and exits without flashing; assign unique IPs (`set_network_config`)
  and re-run.
- **Skip unchanged** — every domain (bootloader/driver/firmware) is compared: the
  bundle's `git_version` vs the sensor's installed hash from `GetDeviceInfo`
  (`bootloader_hash`/`driver_hash`/`fw_hash`); matching domains are skipped and
  only differing ones are flashed. A `-dirty` version always re-flashes.
  `--force` (or `--components`) disables the skip.
- **Apply order** — `bootloader → driver → firmware`. The bootloader is included
  by default but, like the others, is flashed only when its hash differs; flashing
  it is inherently riskier, so review before running with `--force`.
- **Reboot + verify** — after a sensor's domains are flashed, it reboots once,
  then discovery + `GetDeviceInfo` confirm the firmware hash (firmware is
  authoritative; driver/bootloader are advisory).

### Python API (`afi_sdk.software_update`)

| Function | Purpose |
|----------|---------|
| `update_sensor(host, bundle, password, ...)` | Flash one sensor from a bundle; returns a `SensorResult` (does not reboot) |
| `update_sensor_single(host, package, password, ...)` | Flash one sensor from a single-domain package |
| `reboot(host, port=30500)` | Send the reboot command |
| `verify_target(host, expected, ...)` | Wait for reboot, then confirm flashed hashes |
| `is_software_bundle(path)` | True for a `.tar` carrying `software_manifest.json` |
| `flash_domains_for(explicit=None)` | Domains a bundle apply considers (all three by default; each still hash-skipped) |

> **Security:** software update flashes firmware over the plaintext Config API.
> Run it only on a trusted, isolated network, and keep the bundle password secret.

...
| Dependencies | None. `nlohmann/json` 3.12.0 is bundled. | None. Uses only the standard library. |
...
Up to **4096 detections per frame** (36-byte header + 51 bytes per detection). Large RDI frames are segmented with SOME/IP-TP and reassembled automatically by the SDK.
